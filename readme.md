# 11.9更新：
1.加入自动获取群聊图片功能，可在需要时修改代码进行额外操作。
![image](https://github.com/user-attachments/assets/1e3c08f4-0e38-409e-a4e7-85c4bd1a2e2e)
（这是正常情况下自动获取图片的样例，默认只存一个）
2.加入视觉模块，可以识别群聊中发送的png，jpg，jpeg格式的图片，进行解析或拟人情感回复。同时为避免频繁识图，导致token过度消耗，触发视觉模块功能条件将低概率触发，仅在出现“识图”，“解析”等关键词时百分百调用（可自行调整触发条件和概率）。

3.加入视频生成功能，使用指令#生成视频，即可将识别的上一张图片尝试进行视频化，目前推荐视频生成模型为智谱清言的CogVideoX（成本较高），生成过程需等待150秒左右，默认生成6秒左右视频，支持图文一起请求。
